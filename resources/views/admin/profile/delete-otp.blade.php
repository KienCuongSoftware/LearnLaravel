...
<div class="page-header">
    ...
    <a class="btn btn-outline-secondary" href="{{ route('admin.profile.delete.confirm') }}">Quay lại</a>
</div>

<div class="card shadow-sm border-danger">
    <div class="card-body">
        <p class="text-muted">Nhập mã OTP 6 số đã gửi đến email <strong>{{ $user->email }}</strong>.</p>
        <form method="POST" action="{{ route('admin.profile.delete.otp.verify') }}">
            @csrf
        ...
        <form method="POST" action="{{ route('admin.profile.delete.otp.resend') }}" class="mt-3 mb-0">
            @csrf
